...
// Job state lives here (module-level Map). Most transitions (via setJobState) are mirrored
// to chrome.storage.session, so a popup reopened after the service worker was recycled can
// still render recent/in-flight jobs (session storage survives a worker restart, unlike the
// Map)- loadJobsFromSession() re-seeds the Map from it on startup. A brand-new "queued" job
// (submitWord) is written straight into the Map and isn't mirrored until its first
// setJobState transition.
const jobs = new Map();
const MAX_JOBS = 20;
...
function broadcastJobUpdate(job) {
	chrome.runtime.sendMessage({ type: "jobUpdate", job }).catch(() => {
		// No popup listening right now- fine, a reopened popup asks for state via
		// {type: "getState"}, answered from the in-memory `jobs` Map (see the listener
		// below), not from chrome.storage.session directly.
	});
}
...
async function handleGenerateExport(job) {
	...
	try {
		...
		notify(
			`${job.id}-gen-error`,
			"Generation failed",
			`Could not generate a card for "${job.word}": could not reach the backend.`,
		);
		return;
	}
	...
	if (!response.ok) {
		...
		notify(
			`${job.id}-gen-error`,
			"Generation failed",
			`Could not generate a card for "${job.word}": ${detail}`,
		);
		return;
	}
		...
		for await (const event of readNdjsonLines(response)) {
			...
			if (event.event === "error" && event.stage === "generate") {
				...
				notify(
					`${job.id}-gen-error`,
					"Generation failed",
					`Could not generate a card for "${job.word}": ${event.detail}`,
				);
				return;
			}
			...
			if (event.event === "exported") {
				...
				notify(
					`${job.id}-done`,
					"Exported to Anki",
					`"${job.word}" was generated and exported.`,
				);
				return;
			}
		}
