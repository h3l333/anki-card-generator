...
- OpenRouter is the sole LLM provider (see `PROMPTS.md` change log for the prior Gemini/Ollama history)- `OPENROUTER_MODEL` picks which underlying model OpenRouter routes to.
...
- Not yet verified end-to-end against a real OpenRouter response (no API key available in the dev/test environment this was built in)- the request/error-handling paths are confirmed working, but actual generated card quality/latency from OpenRouter hasn't been observed yet.
