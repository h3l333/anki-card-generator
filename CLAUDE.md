...
- `JLPT_LEVEL_DEFAULT`: default `N3`; fallback target learner level (see `backend/llm.py::JLPT_LEVEL_DEFAULT` and the bullet below) used when a `/generate` or `/generate/batch` request doesn't specify `level` itself.
- `ANKICONNECT_URL`: default `http://localhost:8765`.
- `ANKI_DECK_NAME` / `ANKI_NOTE_TYPE`: default `Japanese` / mode-dependent (`Japanese Note Type` in `full` mode, `Basic` in `basic` mode- see `ANKI_EXPORT_MODE` and the field-folding note below).
- `ANKI_EXPORT_MODE`: `full` (default) or `basic`; see the field-folding note below.
...
- `backend/anki.py`: `ANKI_EXPORT_MODE=full` (default) sends each of the eight card fields individually to a custom note type (see README "Configuration"); `export_card` calls `_ensure_full_mode_note_type` first in that mode, which checks AnkiConnect's `modelNames` for `ANKI_NOTE_TYPE` and creates it via `createModel` (eight fields, basic front/back template) if it's missing, leaving an existing note type of that name untouched. `ANKI_EXPORT_MODE=basic` instead folds all eight card fields down into just `Front`/`Back` HTML (`_build_fields`), for Anki's stock `Basic` note type, which only has two fields- a deliberate fallback, not a bug. `ANKI_NOTE_TYPE`'s own default is mode-dependent (`Japanese Note Type` for `full` mode, `Basic` for `basic` mode) precisely so the auto-create path has a name to work with when the user hasn't set `ANKI_NOTE_TYPE` explicitly- override the env var to use a different name. Confirmed working end-to-end (real `addNote` call reaching a running Anki instance via AnkiConnect, card appears in the deck), re-verified 2026-08-11 against the current eight-field shape including `synonyms`/`antonyms`.
- `backend/db.py`: SQLAlchemy engine/session setup plus the ORM models and access functions for the schema `DATABASE.md` documents (`Word`, `Card`, `Export`; `init_db`, `insert_word`, `insert_card`, `find_word_by_kanji`, `get_card`, `get_latest_export`, `record_export`). `Card` rows are write-once for their generated content (inserted before the user edits anything- see `ARCHITECTURE.md`); `record_export()` does flip `Card.exported` to `True` afterwards, so it's the content columns that never change post-insert, not literally every column. `Export` is an append-only history of AnkiConnect calls, used to decide `addNote` vs `updateNoteFields` on a re-export. `init_db()` is called automatically on startup via a FastAPI `lifespan` hook in `backend/main.py`, so no manual step is needed after the Postgres container is up. Wired into **both** the single-word and batch flows for persistence, duplicate-checking, and export tracking: `/generate` calls `find_word_by_kanji`/`get_card`/`insert_word`/`insert_card`; `/generate/batch` does the same per word- calling `find_word_by_kanji`/`get_card` before generating, returning a `duplicate=True` result straight from Postgres on a hit, and only calling `insert_word`/`insert_card` for genuinely new words (see `BatchCardResult` in `backend/models.py`); `/export` calls `get_latest_export`/`record_export` whenever the request carries a `word_id`, single-word or batch alike (see `ExportRequest` in `backend/models.py`, `currentWordId` in `frontend/index.js`, and `cardWordId` in `buildCarouselCard()`). `Word` has a `level` column and both `find_word_by_kanji(kanji, level)` and `insert_word(kanji, reading, source, level)` require it- the duplicate-check key is `(kanji, level)` together, not kanji alone, so the same word regenerated at a different JLPT level is a distinct `Word` row rather than reusing a card written for a different audience. **`create_all()` won't retrofit the `level` column onto a `words` table that already exists from before this change**- needs a hand-run `ALTER TABLE words ADD COLUMN level TEXT NOT NULL DEFAULT 'N3';` or dropping/recreating the table, since there's no migration tool (see `ROADMAP.md`). `tests/test_db.py` exercises this module directly against a real database (the same `DATABASE_URL` the app itself connects to, per docker-compose.yml's defaults- requires `docker compose up -d` running first), covering `insert_word`/`find_word_by_kanji` (including the `(kanji, level)` duplicate-check key), `insert_card`/`get_card`, and `record_export`/`get_latest_export`.
...
- Batch `.txt` upload: `backend/batch.py::parse_and_validate` enforces the format (one word per line, kanji/hiragana/katakana only, max `MAX_WORDS = 12`) and raises `BatchValidationError` for the whole file on any violation. That's a single 400 response, not per-line. `POST /generate/batch` streams its response as newline-delimited JSON rather than one JSON document (`backend/main.py::_stream_batch_results`, returned via `StreamingResponse`, `media_type="application/x-ndjson"`): a leading `{"total": N}` line, then one `{"completed", "total", "result"}` line per word, in the uploaded file's original order, as each word finishes- `llm.generate_cards_batch` is a generator (not a function returning a full list) for exactly this reason, so word N+1 doesn't start generating before word N's result has already been streamed out. It loops `generate_card` per word and catches `LLMError` per word (skip-and-continue) rather than failing the whole batch. Each word's result carries either `card` or `error` in `BatchCardResult`. The route checks each word against Postgres via `find_word_by_kanji` before generating (see the `backend/db.py` note above), then persists every newly-generated result (`insert_word`/`insert_card`, mirroring `/generate`) and sets that result's `word_id`. `frontend/index.js` reads the streamed response incrementally (`response.body.getReader()`), updating a live "X/N done" progress counter and appending one carousel card per result (`.carousel` / `.carousel-card`, horizontally scrollable) as it arrives, rather than waiting for the whole batch; each successful card gets its own independent Discard/Export controls, same as the single-word flow, including its own `word_id` threaded into that card's Export payload. There's no bulk export. Once `StreamingResponse` starts sending bytes the HTTP status is committed to 200, so file-validation errors (`BatchValidationError` -> 400) are raised before streaming begins, not from inside it- an unexpected error mid-stream (e.g. Postgres dropping mid-batch) would just cut the stream short rather than surfacing as a clean error response, an accepted trade-off for this single-user local tool.
