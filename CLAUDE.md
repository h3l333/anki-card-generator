...
- `backend/db.py`: SQLAlchemy engine/session setup plus the ORM models and access functions for the schema `DATABASE.md` documents (`Word`, `Card`, `Export`; `init_db`, `insert_word`, `insert_card`, `find_word_by_kanji`, `get_card`, `get_latest_export`, `record_export`). `Card` rows are write-once (the raw LLM output, inserted before the user edits anything- see `ARCHITECTURE.md`); `Export` is an append-only history of AnkiConnect calls, used to decide `addNote` vs `updateNoteFields` on a re-export. `init_db()` still isn't called from anywhere automatically (no FastAPI startup hook)- run it manually once (`python -c "from backend.db import init_db; init_db()"`) after the Postgres container is up. Fully wired into the **single-word** flow: `/generate` calls `find_word_by_kanji`/`get_card`/`insert_word`/`insert_card`; `/export` calls `get_latest_export`/`record_export` when the request carries a `word_id` (see `ExportRequest` in `backend/models.py` and `currentWordId` in `frontend/index.js`). **Not** wired into the batch flow at all- `/generate/batch`, `generate_cards_batch()`, and `BatchCardResult` never touch Postgres, so batch-generated cards have no `word_id` and a batch export always falls back to a plain `addNote` with nothing recorded. No `tests/test_db.py` yet exercising this module directly against a real database.
...
- `DATABASE.md`: schema (`words`/`cards`/`exports`) implemented in `backend/db.py`, ahead of `backend/main.py` actually calling into it.
