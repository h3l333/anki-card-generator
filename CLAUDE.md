...
- `backend/db.py`: SQLAlchemy engine/session setup plus the ORM models and access functions for the schema `DATABASE.md` documents (`Word`, `Card`, `Export`; `init_db`, `insert_word`, `insert_card`, `find_word_by_kanji`, `get_card`, `get_latest_export`, `record_export`). `Card` rows are write-once for their generated content (inserted before the user edits anything- see `ARCHITECTURE.md`); `record_export()` does flip `Card.exported` to `True` afterwards, so it's the content columns that never change post-insert, not literally every column. `Export` is an append-only history of AnkiConnect calls, used to decide `addNote` vs `updateNoteFields` on a re-export. `init_db()` still isn't called from anywhere automatically (no FastAPI startup hook)- run it manually once (`python -c "from backend.db import init_db; init_db()"`) after the Postgres container is up. Wired into **both** the single-word and batch flows for persistence and export tracking: `/generate` calls `find_word_by_kanji`/`get_card`/`insert_word`/`insert_card`; `/generate/batch` calls `insert_word`/`insert_card` for every successfully-generated result and sets that result's `word_id` (see `BatchCardResult` in `backend/models.py`); `/export` calls `get_latest_export`/`record_export` whenever the request carries a `word_id`, single-word or batch alike (see `ExportRequest` in `backend/models.py`, `currentWordId` in `frontend/index.js`, and `cardWordId` in `buildCarouselCard()`). **Not** wired into batch's *generation* side, though- `/generate/batch` never calls `find_word_by_kanji`/`get_card`, so unlike `/generate` it has no pre-generation duplicate check: every word in a batch file gets a fresh LLM call and a fresh `words`/`cards` row regardless of whether it's already in Postgres (see `ROADMAP.md` for that open decision). No `tests/test_db.py` yet exercising this module directly against a real database.
...
- Batch `.txt` upload: `backend/batch.py::parse_and_validate` enforces the format (one word per line, kanji/hiragana/katakana only, max `MAX_WORDS = 12`) and raises `BatchValidationError` for the whole file on any violation. That's a single 400 response, not per-line. `POST /generate/batch` then calls `llm.generate_cards_batch`, which loops `generate_card` per word and catches `LLMError` per word (skip-and-continue) rather than failing the whole batch. Each word's result carries either `card` or `error` in `BatchCardResult`. The route then persists every successful result (`insert_word`/`insert_card`, mirroring `/generate`) and sets that result's `word_id`- but only *after* generation; there's still no pre-generation duplicate check for batch (see the `backend/db.py` note above). `frontend/index.js` renders one carousel card per result (`.carousel` / `.carousel-card`, horizontally scrollable); each successful card gets its own independent Discard/Export controls, same as the single-word flow, including its own `word_id` threaded into that card's Export payload. There's no bulk export.
